...
class LimoMissionUnified:
    def __init__(self):
        ...
        # =========================
        # (C) GAP follower (미션3) [수정됨: 최소 유지 시간]
        # =========================
        self.gap_speed = 0.22
        self.gap_kp = 1.5
        self.safe_dist = 1.0
        ...
        # [NEW] GAP 모드 강제 유지 시간 (초)
        # 장애물 진입하자마자 M4로 튀는거 방지하기 위해 3초간은 무조건 GAP 유지
        ...
        # =========================
        # (E) M4 Priority Avoid (카메라 기반 - 라바콘) [수정됨: 기준 강화]
        # =========================
        self.img_width = 320
        self.img_height = 240
        self.m4_speed = 0.20
        ...
        # [수정] 노이즈 방지 위해 임계값 상향 (50 -> 150)
        self.detect_thresh = 150 

        self.avoid_level = 0
        self.avoid_cnt = 0
        # [수정] 확실할 때만 넘어가도록 카운트 상향 (6 -> 15)
        self.avoid_enter = 15 
        self.avoid_exit = 10
    ...
    def control_loop(self, _evt):
        ...
        if self.mode == "GAP":
            # [수정] GAP 모드 진입 후 최소 3초간은 M4로 넘어가지 않음 (장애물 회피 강제 수행)
            time_in_gap = now - self.last_mode_change
            
            if time_in_gap > self.gap_min_duration:
                # [디버깅] 현재 M4 진입 조건 충족도 확인
                # rospy.loginfo_throttle(0.5, f"[GAP] AvoidCnt: {self.avoid_cnt}/{self.avoid_enter}")
