...
class LimoMissionUnified:
    def __init__(self):
        ...
        # State
        # =========================
        self.mode = "LINE"  # LINE -> GAP -> M4 -> EDGE_LANE
        self.last_mode_change = rospy.Time.now().to_sec()

        # =========================
        # [수정됨] (A) LINE (검은색 트랙 추종 파라미터 - Code 2 기반)
        # =========================
        self.line_forward_speed = 0.12     # 기본 전진 속도
        self.line_search_speed = 0.25      # 못 찾았을 때 회전 속도
        self.line_k_angle = 0.010          # 조향 게인
        self.line_min_pixels = 5           # 최소 픽셀 수
        self.line_dark_ratio = 0.3         # 임계값 비율
        ...
        rospy.loginfo("Unified node started (BlackLine -> GAP -> M4 -> EDGE).")
    ...
    # ---------------------------------------------------
    # [함수] 검은색 트랙 추종 로직 (Code 2 로직 적용)
    # ---------------------------------------------------
    def process_black_lane(self, img):
        t = Twist()
        h, w, _ = img.shape
        center = w / 2.0
        ...
        # 트랙 못 찾음 -> 회전
        if max_val < self.line_min_pixels:
            t.linear.x = 0.0
            t.angular.z = self.line_search_speed
            self.line_seen = False
            return t

        # 6) 후보 열 추출
        threshold_val = max(self.line_min_pixels, int(max_val * self.line_dark_ratio))
        ...
        # 8) 조향 계산 (왼쪽+, 오른쪽-)
        ang = -self.line_k_angle * offset
        ang = max(min(ang, 0.8), -0.8)

        t.linear.x = self.line_forward_speed
        t.angular.z = ang
    ...
    def image_callback(self, msg):
        ...
        # ===== (1) LINE controller (검은색 트랙 추종으로 변경됨) =====
        # [기존 흰색 로직 삭제됨] -> [Code 2의 검은색 로직 사용]
        self.line_twist = self.process_black_lane(img)
        ...
        # ===== (3) Edge Lane Controller (미션 5/6용) =====
        # 여기도 동일하게 검은색 트랙 로직을 사용합니다.
        self.edge_twist = self.process_black_lane(img)

    # -------------------------
    # LiDAR callback (수정 금지)
    # -------------------------
    ...
    def control_loop(self, _evt):
        ...
        if self.mode == "LINE":
            rospy.loginfo_throttle(0.5, f"[LINE] Front: {self.front_min:.2f}m | Cnt: {self.trigger_count}/{self.trigger_limit}")
            if self.trigger_count >= self.trigger_limit:
                self.mode = "GAP"
                self.last_mode_change = now
                self.trigger_count = 0
                rospy.loginfo("SWITCH: LINE -> GAP (Obstacle Detected)")
                stop = Twist()
                for _ in range(5):
        ...
        if self.mode == "GAP":
            time_in_gap = now - self.last_mode_change
            
            if time_in_gap > self.gap_min_duration:
