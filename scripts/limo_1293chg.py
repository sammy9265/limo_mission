    ...
    def camera_cb(self, msg):
        ...
        if self.state == "LANE":
            ...
            try:
                ...
                # --------------------------------------------------
                # [PRIORITY 1] 빨간색 감지 및 회피 (Red Priority)
                # --------------------------------------------------
                ...
                # --------------------------------------------------
                # [PRIORITY 2] 검은색 트랙 추종 (EdgeLaneNoBridge 로직 적용)
                # --------------------------------------------------
                ...
                # 5) 열별 픽셀 수 계산
                mask_bin = (binary > 0)
                col_sum = np.sum(mask_bin, axis=0) # shape: (w,)
                max_val = int(np.max(col_sum)) if col_sum.size > 0 else 0

                # 6) 검은색 라인이 너무 적으면 -> 못 찾음 -> 회전
                ...
                # 왼쪽(+), 오른쪽(-) 부호 유지 (EdgeLaneNoBridge 로직)
